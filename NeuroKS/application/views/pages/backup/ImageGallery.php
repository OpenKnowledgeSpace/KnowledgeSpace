...
<?php
	     ...
	     {
                //$hasImageArray[0]=$cilImages[0];
                array_push($hasImageArray,($cilImages[0]));
                echo "<div class=\"col-md-3\">";
		echo $cilImages[0];
                echo "</div>";
                $imageCount++;
...
<?php
        ...
        if(!$hasFirstImage && sizeof($cilImages) > 1)
        {
            ...
            //$hasImageArray[1]=$cilImages[count($originalCILImages)-1];
            array_push($hasImageArray,($cilImages[count($originalCILImages)-1]));
            echo "</div>";
            $hasFirstImage = true;
            $imageCount++;
             }
...
<?php
		...
		if( $neuroMorphoImages != null && sizeof($neuroMorphoImages) > 0)
		{
                    for($i=0;$i<sizeof($neuroMorphoImages);$i++)
                    {
                        ...
                        if(!in_array($neuroMorphoImages[$i], $hasImageArray))
                        {
                        echo "<div class=\"col-md-3\">";
			echo $neuroMorphoImages[$i];
                        array_push($hasImageArray, $neuroMorphoImages[$i]);
                        echo "</div> ";
...
<?php
		...
		if($imageCount < 3 && sizeof($cilImages) > 0)
		{
                    ...
                    {
                        if(!in_array($cilImages[$i], $hasImageArray))
                        {
                            echo "<div class=\"col-md-3\">";
                            echo $cilImages[$i];
                            array_push($hasImageArray, $cilImages[$i]);
                            echo "</div> ";
